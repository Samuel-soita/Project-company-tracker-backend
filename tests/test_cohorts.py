...
def test_cohort_crud(client, app):
    # Login manager - sets cookie automatically
    login = client.post('/auth/login', json={'email': 'manager@test.com', 'password': 'adminpass'})
    assert login.status_code == 200

    # Create cohort
    res = client.post('/cohorts/', json={'name': 'Cohort 1', 'description': 'Desc'})
    ...
    res = client.put(f'/cohorts/{cohort_id}', json=updated_data, )
    assert res.status_code == 200
    ...
    # Delete cohort
    res = client.delete(f'/cohorts/{cohort_id}', )
    assert res.status_code == 200
